...
def main():
    ...
    open_reads(r1, r2, read_dt, compressed)


def make_empty_reads(og_ls):
    for og in og_ls:
        with open(f"{og}_R1.fq", 'w'), open(f"{og}_R2.fq", 'w'):
            pass
...
def open_reads(r1, r2, read_dt, compressed):
    '''
    ...
    if compressed:
        with gzip.open(r1, 'rt') as f1, gzip.open(r2, 'rt') as f2:
            iterate_reads(f1, f2, read_dt)
    else:
        with open(r1) as f1, open(r2) as f2:
            iterate_reads(f1, f2, read_dt)


def iterate_reads(f1, f2, read_dt):
    write_line = False

    for idx, (line1, line2) in enumerate(zip(f1, f2)):
        if (idx + 4) % 4 == 0:
            if write_line:
                write_to_file(seq1, seq2, og_ls)
            write_line = False
            seq1, seq2 = "", ""
...
def write_to_file(seq1, seq2, og_ls):
    for og in og_ls:
        with open(f"{og}_R1.fq", 'a') as f1, open(f"{og}_R2.fq", 'a') as f2:
            f1.write(seq1)
            f2.write(seq2)
